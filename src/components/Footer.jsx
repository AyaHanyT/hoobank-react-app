          ...
          <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
            ...
            <ul className='list-none mt-4'>
              {footerLink.links.map((Link, index) => (
                <li key={Link.name} className={`${index !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'} font-poppins font-normal text-[16px] text-dimWhite hover:text-secondary leading-[24px] cursor-pointer`}>
                  {Link.name}
                </li>
              ))}
            </ul>
